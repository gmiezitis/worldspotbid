...
import { BID_INCREMENT_CENTS, COLOR_BORDER_ADDON_CENTS } from '../../../lib/bidding';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.colorfulBorder !== undefined && typeof body.colorfulBorder !== 'boolean') throw new ApiError(400, 'Invalid border option.');
    const colorfulBorder = body.colorfulBorder === true;
    const borderAddonCents = colorfulBorder ? COLOR_BORDER_ADDON_CENTS : 0;
    let logoKey = '';
    ...
    await db.prepare(`
      INSERT INTO bid_orders (
        ...
        expected_version, company_name, company_url, business_description, project_category, charity_cause,
        colorful_border, border_addon_cents, logo_key, status, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'creating_checkout', ?, ?)
    `).bind(orderId, country.code, country.name, user.userId, user.email, amountCents, current.version, companyName, companyUrl, businessDescription, projectCategory, charityCause, colorfulBorder ? 1 : 0, borderAddonCents, logoKey, now, now).run();
    ...
    const session = await getStripe().checkout.sessions.create({
      ...
      metadata: { orderId, countryCode: country.code, expectedVersion: String(current.version), charityCause, colorfulBorder: String(colorfulBorder) },
      payment_intent_data: {
        capture_method: 'manual',
        metadata: { orderId, countryCode: country.code, charityCause, colorfulBorder: String(colorfulBorder) },
      },
      line_items: [{
        quantity: 1,
        price_data: {
        ...
        price_data: {
          currency: 'usd',
          unit_amount: COLOR_BORDER_ADDON_CENTS,
          product_data: {
            name: 'Colorful country border',
            description: 'A high-visibility multicolor border shown around the country while this placement is active.',
          },
        },
      }] : [])],
    ...
    return Response.json({ url: session.url, amount: (amountCents + borderAddonCents) / 100 });
  } catch (error) {
    if (orderId) {
